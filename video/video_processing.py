...
def video_creation_with_statusbar(path_image: str, path_audio: str, file_path_pptx: str, codec: str):
...
def video_creation_without_statusbar(path_image: str, path_audio: str, file_path_pptx: str, codec: str):
    # Список файлов изображений
    image_files = sorted(os.listdir(path_image), key=lambda x: int(x.split('_')[1][:-4]))
    ...
    final_clip.write_videofile(
        ...
        threads=num_threads,  # Используем все доступные ядра
        preset='medium',  # Баланс между скоростью и качеством
        verbose=False,  # Отключаем вывод информации о процессе
