...
from dictionaries.dictionaries_processing import numbers_dict, numbers_dict2, numbers_dict_date
...
def replace_temp(text, my_numbers_dict_date, my_numbers_dict):
    word_replacements = {
    ...
    for match in matches:
        ...
        for key, value in my_numbers_dict_date.items():
            if key == date:
                date = value
        for key_2, value_2 in my_numbers_dict.items():
            if key_2 == time:
                time = value_2
        if max_min == 'X':
...
def replace_temp2(text, my_numbers_dict_date, my_numbers_dict):
    word_replacements = {
        "11 градус,": "11 градусов",
        "12 градус,": "12 градусов",
        "13 градус,": "13 градусов",
...
def replace_from(text, my_numbers_dict):
    # Ищем все значения с MPS в тексте
